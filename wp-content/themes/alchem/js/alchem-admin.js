  ...
  ALCHEM_UI = {
    ...
    init_add: function() {
      ...
      $(document).on('click', '.option-tree-list-item-setting-add', function(e) {
        ...
        ALCHEM_UI.add(this,'list_item_setting');
      });
    },
    init_edit: function() {
      $(document).on('click', '.option-tree-setting-edit', function(e) {
        ...
        if ( $(this).parents().hasClass('option-tree-setting-body') ) {
          ALCHEM_UI.init_remove_active($(this),'child');
          ALCHEM_UI.init_hide_body($(this),'child');
        } else {
          ALCHEM_UI.init_remove_active($(this),'parent');
          ALCHEM_UI.init_hide_body($(this), 'parent');
        }
        $(this).toggleClass('active');
        ALCHEM_UI.init_hide_body($(this), 'toggle');
      });
    },
    init_remove: function() {
      $(document).on('click', '.option-tree-setting-remove', function(event) {
        ...
        if (agree) {
          ...
          ALCHEM_UI.remove(this);
          setTimeout( function() { 
            ALCHEM_UI.update_ids(list); 
          }, 200 );
        }
        return false;
      });
        ...
        $.ajax({
          ...
          complete: function( data ) {
            if ( type == 'choice' || type == 'list_item_setting' ) {
              ALCHEM_UI.init_remove_active(list,'child-add');
              ALCHEM_UI.init_hide_body(list,'child-add');
            } else {
              ALCHEM_UI.init_remove_active();
              ALCHEM_UI.init_hide_body();
            }
            ...
            setTimeout( function() {
              ALCHEM_UI.init_sortable();
              ALCHEM_UI.init_select_wrapper();
              ALCHEM_UI.init_numeric_slider();
              ALCHEM_UI.parse_condition();
            }, 500);
    ...
    update_id: function(e) {
      ...
      this.timer = setTimeout( function() {
        ALCHEM_UI.update_ids($(e).parents('ul'));
      }, 100);
      return true;
    },
    ...
    parse_condition: function() {
      $( '.format-settings[id^="setting_"][data-condition]' ).each(function() {
        ...
        var passed;
        var conditions = ALCHEM_UI.match_conditions( $( this ).data( 'condition' ) );
        var operator = ( $( this ).data( 'operator' ) || 'and' ).toLowerCase();

        $.each( conditions, function( index, condition ) {

          var target   = $( '#setting_' + condition.check );
          var targetEl = !! target.length && target.find( 'select, input[type="radio"]:checked, input[type="text"], input[type="hidden"], input.alchem-numeric-slider-hidden-input' ).first();
    ...
    init_conditions: function() {
      $( document ).on( 'change.conditionals', '.format-settings[id^="setting_"] select, .format-settings[id^="setting_"] input[type="radio"]:checked, .format-settings[id^="setting_"] input[type="text"], .format-settings[id^="setting_"] input[type="hidden"], .format-settings[id^="setting_"] input.alchem-numeric-slider-hidden-input', function( e ) {
        ALCHEM_UI.parse_condition();
      });
      ALCHEM_UI.parse_condition();
    },
    init_upload: function() {
      $(document).on('click', '.alchem_upload_media', function() {
        var field_id    = $(this).parent('.option-tree-ui-upload-parent').find('input').attr('id'),
            post_id     = $(this).attr('rel'),
            btnContent  = '';
        if ( window.wp && wp.media ) {
          window.alchem_media_frame = window.alchem_media_frame || new wp.media.view.MediaFrame.Select({
          ...
          window.alchem_media_frame.on('select', function() {
            var attachment = window.alchem_media_frame.state().get('selection').first(), 
            ...
            btnContent += '<a href="javascript:(void);" class="option-tree-ui-remove-media option-tree-ui-button button button-secondary light" title="'+option_tree.remove_media_text+'"><span class="icon alchem-icon-minus-sign"></span>'+option_tree.remove_media_text+'</a>';
            $('#'+field_id).val(href);
            ...
            window.alchem_media_frame.off('select');
          }).open();
        } else {
          ...
          window.send_to_editor = function(html) {
            ...
            if (href.match(image) && ALCHEM_UI.url_exists(href)) {
              btnContent += '<div class="option-tree-ui-image-wrap"><img src="'+href+'" alt="" /></div>';
            }
            btnContent += '<a href="javascript:(void);" class="option-tree-ui-remove-media option-tree-ui-button button button-secondary light" title="'+option_tree.remove_media_text+'"><span class="icon alchem-icon-minus-sign"></span>'+option_tree.remove_media_text+'</a>';
            $('#'+field_id).val(href);
    ...
    init_upload_fix: function(elm) {
      ...
      if ( val !== '' && ( typeof src == 'undefined' || src == false ) && ALCHEM_UI.url_exists(val) ) {
        var image = /\.(?:jpe?g|png|gif|ico)$/i;
        if (val.match(image)) {
          btnContent += '<div class="option-tree-ui-image-wrap"><img src="'+val+'" alt="" /></div>';
        }
        btnContent += '<a href="javascript:(void);" class="option-tree-ui-remove-media option-tree-ui-button button button-secondary light" title="'+option_tree.remove_media_text+'"><span class="icon alchem-icon-minus-sign">'+option_tree.remove_media_text+'</span></a>';
        $('#'+id).val(val);
        $('#'+id+'_media').remove();
        $('#'+id).parent().parent('div').append('<div class="option-tree-ui-media-wrap" id="'+id+'_media" />');
        $('#'+id+'_media').append(btnContent).slideDown();
      } else if ( val == '' || ! ALCHEM_UI.url_exists(val) ) {
        $(elm).parent().next('.option-tree-ui-media-wrap').remove();
      }
    },
    init_numeric_slider: function() {
      $(".alchem-numeric-slider-wrap").each(function() {
        var hidden = $(".alchem-numeric-slider-hidden-input", this),
            value  = hidden.val(),
            helper = $(".alchem-numeric-slider-helper-input", this);
        if ( ! value ) {
          value = hidden.data("min");
          helper.val(value)
        }
        $(".alchem-numeric-slider", this).slider({
          min: hidden.data("min"),
          max: hidden.data("max"),
    ...
    fix_upload_parent: function() {
      $('.option-tree-ui-upload-input').on('focus blur', function(){
        ...
        ALCHEM_UI.init_upload_fix(this);
      });
    },
    ...
    css_editor_mode: function() {
      $('.alchem-css-editor').each(function() {
...
!function ($) {
  ...
  alchem_gallery = {
    ...
    frame: function (elm) {
      ...
      this._frame.on('update', function () {
        ...
        var controller = alchem_gallery._frame.states.get('gallery-edit')
          , library = controller.get('library')
        , ids = library.pluck('id')
          , parent = $(elm).parents('.format-setting-inner')
          , input = parent.children('.alchem-gallery-value')
          , shortcode = wp.media.gallery.shortcode( selection ).string().replace(/\"/g,"'");
       
        input.attr('value', ids);
                       
        if ( parent.children('.alchem-gallery-list').length <= 0 )
          input.after('<ul class="alchem-gallery-list" />')
        ...
        $.ajax({
          ...
          success: function(res) {
            parent.children('.alchem-gallery-list').html(res)
            if ( input.hasClass('alchem-gallery-shortcode') ) 
              input.val(shortcode);
            if ( $(elm).parent().children('.alchem-gallery-delete').length <= 0 ) {
              $(elm).parent().append('<a href="#" class="option-tree-ui-button button button-secondary hug-left alchem-gallery-delete">' + option_tree.delete + '</a>')
            }
  ...
  , select: function (elm) {
	  ...
	  var _alchem_gallery =  wp.media.view.settings.alchem_gallery;
	  if( typeof _alchem_gallery=== "undefined")
	  _alchem_gallery = "";
	
      var input = $(elm).parents('.format-setting-inner').children('.alchem-gallery-value')
        , ids   = input.attr('value')
        , _shortcode = input.hasClass('alchem-gallery-shortcode') ? ids : '[gallery ids=\'' + ids + '\]'
        , shortcode  = wp.shortcode.next('gallery', ( ids ? _shortcode :_alchem_gallery.shortcode ) )
      ...
      if ( _.isUndefined( shortcode.get('ids') ) && ! input.hasClass('alchem-gallery-shortcode') && ids )
  ...
  , remove: function (elm) {
      ...
      if ( confirm( option_tree.confirm ) ) {
        ...
        $(elm).parents('.format-setting-inner').children('.alchem-gallery-value').attr('value', '')
        $(elm).parents('.format-setting-inner').children('.alchem-gallery-list').remove()
        $(elm).next('.alchem-gallery-edit').text( option_tree.create )
        $(elm).remove()
        ALCHEM_UI.init_conditions()
  ...
  // Gallery delete
  $(document).on('click.alchem_gallery.data-api', '.alchem-gallery-delete', function (e) {
    e.preventDefault()
    alchem_gallery.remove($(this))
  })
  
  // Gallery edit
  $(document).on('click.alchem_gallery.data-api', '.alchem-gallery-edit', function (e) {
    e.preventDefault()
    alchem_gallery.open($(this))
  })
...
!function ($) {
  ...
  $(document).on('ready', function () {
    ...
    $('.alchem-metabox-wrapper').each( function() {
    
      // Only if there is a tab option
      if ( $(this).find('.type-tab').length ) {
        
        // Add .alchem-metabox-panels
        $(this).find('.type-tab').parents('.alchem-metabox-wrapper').wrapInner('<div class="alchem-metabox-panels" />')
        
        // Wrapp with .alchem-metabox-tabs & add .alchem-metabox-nav before .alchem-metabox-panels
        $(this).find('.alchem-metabox-panels').wrap('<div class="alchem-metabox-tabs" />').before('<ul class="alchem-metabox-nav" />')
        ...
        $(this).find('.format-settings').each( function() {
          ...
          if ( $(this).find('.type-tab').length > 0 ) {
            ...
            $(this).parents('.alchem-metabox-panels').prev('.alchem-metabox-nav').append('<li><a href="#' + id + '">' + title + '</a></li>')
        ...
        // Create the tabs
        $(this).find('.alchem-metabox-tabs').tabs()
        
        // Move the orphaned settings to the top
        $(this).find('.alchem-metabox-panels > .format-settings').prependTo($(this))
  ...
  $(document).on('ready', function () {
    ...
    $('#option-tree-settings-api .inside').each( function() {
      ...
      if ( $(this).find('.type-tab').length ) {
        ...
        // Add .alchem-theme-option-panels
        $(this).find('.type-tab').parents('.inside').wrapInner('<div class="alchem-theme-option-panels" />')
        
        // Wrap with .alchem-theme-option-tabs & add .alchem-theme-option-nav before .alchem-theme-option-panels
        $(this).find('.alchem-theme-option-panels').wrap('<div class="alchem-theme-option-tabs" />').before('<ul class="alchem-theme-option-nav" />')
        ...
        $(this).find('.format-settings').each( function() {
          ...
          if ( $(this).find('.type-tab').length > 0 ) {
            ...
            // Add a class, hide & append nav item 
            $(this).addClass('is-panel').hide()
            $(this).parents('.alchem-theme-option-panels').prev('.alchem-theme-option-nav').append('<li><a href="#' + id + '">' + title + '</a></li>')
        ...
        // Create the tabs
        $(this).find('.alchem-theme-option-tabs').tabs()
        
        // Move the orphaned settings to the top
        $(this).find('.alchem-theme-option-panels > .format-settings').prependTo($(this).find('.alchem-theme-option-tabs'))
